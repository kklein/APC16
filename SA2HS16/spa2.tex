...
\item
	...
	\begin{enumerate}
		\item \(det|\tilde{A_j}|\) \\
			Let's do a case distiction on \(\tilde{x}\). 
			\begin{enumerate} [a)]
			\item \(\tilde{x}\) satisfies at least one constraint from \(x \geq 0\) with equality.
 			\item \(\tilde{x}\) satisfies all and only constraints from \(Ax \geq 1_m\) with equality.
			\end{enumerate}
		\item \({det|\tilde{A}|}\) \\
