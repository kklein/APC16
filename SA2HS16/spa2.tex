...
\item
	...
	\begin{enumerate}
		\item \(det|\tilde{A_j}|\) \\
			Let's do a case distiction on \(\tilde{x_j}\). 
			\begin{enumerate} [a)]
			\item \(\tilde{x_j}\) satisfies a constraint from \(x \geq 0\) with equality. \\
				We know that this means that there is a row \(i\) in \(\tilde{A}\) which contains a 1 in position \(j\) and else only 0's. Furthermore we know that \( \tilde{b} \) must hold 0 in position \(i\). We can think of it as \(  \begin{bmatrix} 0 & \dots & 0 & 1 & 0 & \dots & 0 \\\end{bmatrix} \cdot \tilde{x} = 0\). Now as the column \(j\) of \(\tilde{A}\) is replaced by \( \tilde{b} \) and the \(i\)th row \(\tilde{A_j}\) is all 0's. The sole 1 in the row, precisely in \(\tilde{A[i][j]}\), is replaced by the 0 in \(\tilde{b_i}\). \\
				We now have a row of 0's in \(\tilde{A_j}\). This, in turn implies that \(\tilde{A_j}\) is not invertible and hence \(det|\tilde{A_j}| = 0\).
 			\item \(\tilde{x_j}\) satisfies a constraint from \(Ax \geq 1_m\) with equality. \\
 				As \(\tilde{A}\tilde{x} = \tilde{b}\), we can express 
			\end{enumerate}
		\item \({det|\tilde{A}|}\) \\
