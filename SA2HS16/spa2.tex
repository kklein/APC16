...
\item
	...
	&= \sum_{i=1}^m \mathbb{E}[\mathbb{I}[f_i\ not\ intersected]] & (linearity\ of\ expectation) \\
	&= \sum_{i=1}^m \Pr[f_i\ not\ intersected] \\
	&= \sum_{i=1}^m (1 - \frac{A_i x^*}{\tau^*(F)})^s \\
	& \leq \sum_{i=1}^m (1 - \frac{1}{\tau^*(F)})^s\ & (Ax^* \geq 1_m ) \\
	\end{align}
	We now have a sum of terms. For each of the terms, we can apply \( 1 + x \leq e^x\).
	\begin{align}
	\mathbb{E}[\# flags\ not\ intersected\ by\ s\ draws] & \leq \sum_{i=1}^m (e^{- \frac{1}{\tau^*(F)}})^s   \\
	& \leq \sum_{i=1}^m e^{- \frac{s}{\tau^*(F)}} \\
	& \leq m \cdot e^{- \frac{s}{\tau^*(F)}}
	\end{align}
	We will investigate what the necessary condition for this term to be less or equal 1 looks like.
	\begin{align}
	...
	\end{align}
	In other words, we know that for any \(s\) larger than \(\tau^*(F) \cdot ln(m) \), the expected number of not intersected flags is smaller than 1. 
	Let's check whether the same property holds for \(s = \tau(F)\).
	\begin{align}
	 \mathbb{E}[\# flags\ not\ intersected\ by\ s\ draws] &= \sum_{i=1}^m (1 - \frac{A_i x^*}{\tau^*(F)})^s \\
	 &=  \sum_{i=1}^m (1 - \frac{A_i x^*}{\tau^*(F)})^{\tau(F)} \\
	 & \geq something something
	\end{align}
%%%% f)
\item
	\begin{enumerate}[(i)]
