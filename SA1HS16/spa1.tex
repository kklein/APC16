...
% Custom commands
\DeclareMathOperator{\Kl}{Kl} %Klassen von ZustÃÂÃÂÃÂÃÂ¤nden
...
\item % 3 b
	...
	\begin{enumerate}[1.] 
		...
		\item \textbf{Correctnes} \\
			We know that we are given the number of inversions between the ${1..n}$ and the permutation $\pi_a$, including permutations in $x=a$. \\
			Hence we need to prove that the amount of inversions in the permutation $\pi_a$ is equal to the number of intersections of lines from $L$, up to and including $x=a$. We will do so by induction. In order to so so, we will first have to prove the very helpful claim that we can look at pairwise intersections without loss of generality. \\
			\textit{Claim.} If there are two lines crossing in one point, the number of inversions in the permutation increases for exactly one in that point.  \\
			\textit{Proof.} Given that we have an intersection of lines $l_1$ and $l_2$ in $x=a$, we know that $a \cdot x_1 + y_1 = a \cdot x_2 + y_2$. As the permutation is a total order, we know that either $x_1 < x_2$ or vice versa. Let's assume $x_1 < x_2$ (the opposite is symmetric), which implies that $\pi_a(l_2) < \pi_a(l_1)$ according to the definition of $\pi_a$. 
				CHECK THIS IFF
				\begin{align}
			...
			\textit{Claim:} Looking at the intersections of lines atomically, i.e. only considering intersections of two lines at a time, does not change the number of resulting inversions in the permutation as compared to considering all lines intersecting in one point together.\\
			\textit{Proof:} 
				Assume that we have k lines intersecting in one point. \\
				When we look at the intersections in a pairwise manner, we realize that we have one inversion per two lines. Therefore we have exactly ${k \choose 2}$ many intersections. Our atomic point of view yields that we have one inversion per intersections, namely ${k \choose 2}$ inversions.
				When we look at the intersection of all lines together
			
			\begin{enumerate}[-]
				\item $x=a$ s.t. the leftmost intersection of a pair of lines is at $x=a$.
				\item We had $k$ intersections for $x=a$ and now look at an additional intersection in $x=a+\delta$ with $\delta \in \mathbb{R}$ minimal. \\
