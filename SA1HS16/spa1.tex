...
\item % 3 c
	...
	make $\pi_c$ represent the set of inversions from $b$ to $c$. Therefore we can feed $\pi_c$ to the algorithm from Fact B and return its result. The runtime remains $\mathcal{O}(nlogn)$ as the algorithm from Fact B takes $\mathcal{O}(nlogn)$ after applying our algorithm of $\mathcal{O}(nlogn)$ from b) twice. 
